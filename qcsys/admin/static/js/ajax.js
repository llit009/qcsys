        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.errormsg == 1){
                    ...
                    $('#checkqc_num').append("<span>&#10006; 编号有误，请重新输入</span>");
...
// Other info Ajax
$(function() {
    $("#otherinfo").autocomplete({
        source:function(request, response) {
            $.getJSON("/admin/otherinfoinput",{
                oinfo: request.term, // in flask, "q" will be the argument to look for using request.args
            }, function(data) {
                console.log(data)
                response(data.matching_result); // matching_results from jsonify
            });
